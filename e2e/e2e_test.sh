#!/usr/bin/env bash
# e2e_test.sh — End-to-end tests for architect-ai installer
#
...
if [ -z "$BINARY" ]; then
    echo "ERROR: architect-ai binary not found. Build it first."
    exit 1
fi
...
test_version_command() {
    ...
    if echo "$output" | grep -q "architect-ai"; then
...
test_cc_engram_injection() {
    ...
    if $BINARY install --agent claude-code --component engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "architect-ai:engram-protocol" "CLAUDE.md has engram-protocol section marker"
...
test_cc_sdd_injection() {
    ...
    if $BINARY install --agent claude-code --component sdd --persona neutral 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "architect-ai:sdd-orchestrator" "CLAUDE.md has SDD section marker"
...
test_cc_persona_gentleman() {
    ...
    if $BINARY install --agent claude-code --component persona --persona architect 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "architect-ai:persona" "CLAUDE.md has persona section marker"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "Architect" "Gentleman persona has 'Architect'"
...
test_cc_persona_neutral() {
    ...
    if $BINARY install --agent claude-code --component persona --persona neutral 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "architect-ai:persona" "CLAUDE.md has persona section marker"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "Architect" "Neutral persona keeps the teacher identity"
...
test_oc_engram_injection() {
    ...
    if $BINARY install --agent opencode --component engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$agents_md" 'architect-ai:engram-protocol' "AGENTS.md has engram-protocol section"
...
test_oc_persona_gentleman() {
    ...
    if $BINARY install --agent opencode --component persona --persona architect 2>&1; then
        local agents_md="$HOME/.config/opencode/AGENTS.md"
        assert_file_exists "$agents_md" "AGENTS.md exists"
        assert_file_contains "$agents_md" "Architect" "Gentleman persona has 'Architect'"
...
test_oc_persona_neutral() {
    ...
    if $BINARY install --agent opencode --component persona --persona neutral 2>&1; then
        local agents_md="$HOME/.config/opencode/AGENTS.md"
        assert_file_exists "$agents_md" "AGENTS.md exists"
        assert_file_contains "$agents_md" "Architect" "Neutral persona keeps the teacher identity"
...
test_full_preset_claude_code() {
    ...
    if $BINARY install --agent claude-code --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-gentleman --persona architect 2>&1; then
        local claude_md="$HOME/.claude/CLAUDE.md"
        local settings="$HOME/.claude/settings.json"

        # CLAUDE.md should have all 3 sections coexisting
        assert_file_exists "$claude_md" "CLAUDE.md exists"
        assert_file_contains "$claude_md" "architect-ai:sdd-orchestrator" "Has SDD section"
        assert_file_contains "$claude_md" "architect-ai:persona" "Has persona section"
...
test_full_preset_opencode() {
    ...
    if $BINARY install --agent opencode --component engram --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-gentleman --persona architect 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
        local agents_md="$HOME/.config/opencode/AGENTS.md"
        ...
        assert_file_exists "$agents_md" "AGENTS.md exists"
        assert_file_contains "$agents_md" "Architect" "Gentleman persona"
        assert_file_contains "$agents_md" "architect-ai:engram-protocol" "AGENTS.md has engram protocol"
        ...
        assert_file_not_contains "$agents_md" "<!-- architect-ai:sdd-orchestrator -->" "AGENTS.md has no SDD section marker (opencode uses json agent)"
...
test_minimal_preset_opencode_only_engram_no_persona() {
    ...
    if $BINARY install --agent opencode --preset minimal --persona neutral 2>&1; then
        ...
        if [ -f "$agents_md" ]; then
            assert_file_not_contains "$agents_md" "architect-ai:persona" "No persona marker in minimal preset"
            assert_file_not_contains "$agents_md" "Architect" "No persona content in minimal preset"
        else
...
test_minimal_preset_claude_only_engram() {
    ...
    if $BINARY install --agent claude-code --preset minimal --persona neutral 2>&1; then
        ...
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "architect-ai:engram-protocol" "Engram protocol section"

        # SDD should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "architect-ai:sdd-orchestrator" "No SDD in minimal"
        # Persona should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "architect-ai:persona" "No persona in minimal"
...
test_ecosystem_both_agents() {
    ...
    if $BINARY install --agent claude-code --agent opencode --component sdd --component skills --component context7 --preset ecosystem-only --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "architect-ai:sdd-orchestrator" "Claude has SDD"
...
test_content_claude_md_sections_substantial() {
    ...
    $BINARY install --agent claude-code --component sdd --component persona --persona architect 2>&1 || true
    $BINARY install --agent claude-code --component engram --persona architect 2>&1 || true
...
test_idempotent_persona_claude() {
    ...
    $BINARY install --agent claude-code --component persona --persona architect 2>&1 || true
    $BINARY install --agent claude-code --component persona --persona architect 2>&1 || true
...
test_idempotent_full_claude() {
    ...
    $BINARY install --agent claude-code --component sdd --component persona --component context7 --component permissions --component theme --preset full-gentleman --persona architect 2>&1 || true
    local first_md_hash
...
test_edge_persona_switch() {
    ...
    $BINARY install --agent claude-code --component persona --persona architect 2>&1 || true
    assert_file_contains "$HOME/.claude/CLAUDE.md" "Architect" "First install: gentleman persona"
    ...
    # So "Architect" still appears, but Rioplatense markers are gone.
    $BINARY install --agent claude-code --component persona --persona neutral 2>&1 || true
    assert_file_contains "$HOME/.claude/CLAUDE.md" "Architect" "Second install: neutral still has teacher identity"
...
test_edge_persona_switch_preserves_sections_opencode() {
    ...
    $BINARY install --agent opencode --component persona --component engram --component sdd --persona architect 2>&1 || true

    local agents_md="$HOME/.config/opencode/AGENTS.md"
    assert_file_exists "$agents_md" "AGENTS.md after full install"
    assert_file_contains "$agents_md" "architect-ai:engram-protocol" "Engram section present before switch"
    ...
    assert_file_contains "$agents_md" "Architect" "Neutral persona present after switch"
    assert_file_not_contains "$agents_md" "Rioplatense" "Regional language removed after switch"
    assert_file_contains "$agents_md" "architect-ai:engram-protocol" "Engram section survived persona switch"
...
test_windsurf_persona_and_sdd_content() {
    ...
    if $BINARY install --agent windsurf --component persona --component sdd --persona architect 2>&1; then
        local rules="$HOME/.codeium/windsurf/memories/global_rules.md"
        assert_file_exists "$rules" "global_rules.md exists"
        assert_file_contains "$rules" "Architect" "Persona injected"
        assert_file_contains "$rules" "architect-ai:sdd-orchestrator" "SDD orchestrator marker present"
        ...
        assert_file_contains "$rules" "architect-ai:engram-protocol" "Engram protocol marker present"
...
test_integrity_full_preset_all_skills_nonempty() {
    ...
    if $BINARY install --agent opencode --component sdd --component skills --preset full-gentleman --persona architect 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        local all_ok=true
        local empty_count=0
...
test_integrity_sdd_orchestrator_agent_structure() {
    ...
    if $BINARY install --agent opencode --component sdd --persona architect 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
...
test_backup_created_on_install() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local backup_count
        backup_count=$(find "$HOME/.architect-ai/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
...
test_backup_contains_original_files() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local latest_backup
        latest_backup=$(find "$HOME/.architect-ai/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -1)
