#!/usr/bin/env bash
# lib.sh — shared test helpers for architect-ai E2E tests
...
# Binary resolution
# ---------------------------------------------------------------------------
# The binary should be built and placed at /usr/local/bin/architect-ai inside
# the Docker container. If not found, fall back to $HOME/architect-ai or the
# current directory.
# Resolution priority (highest → lowest):
#   1. ./architect-ai in the current repo directory (freshly built local binary)
#   2. ~/architect-ai (explicit copy in home)
#   3. architect-ai on PATH (system-installed, e.g. Homebrew)
# This ensures `go build ./cmd/architect-ai && bash e2e/e2e_test.sh` always
# tests the locally built binary rather than the installed release version.
resolve_binary() {
    # Prefer the locally built binary (./architect-ai) produced by `go build ./cmd/architect-ai`.
    ...
    if [ -x "$repo_root/architect-ai" ]; then
        echo "$repo_root/architect-ai"
    elif [ -x "./architect-ai" ]; then
        echo "./architect-ai"
    elif [ -x "$HOME/architect-ai" ]; then
        echo "$HOME/architect-ai"
    elif command -v architect-ai >/dev/null 2>&1; then
        echo "architect-ai"
    else
        echo ""
    fi
...
cleanup_test_env() {
    ...
    rm -rf "$HOME/.architect-ai" 2>/dev/null || true
...
# Checks that the architect-ai section marker appears exactly once (no duplicates).
assert_no_duplicate_section() {
    ...
    local marker="<!-- architect-ai:${section_id} -->"
    local count
