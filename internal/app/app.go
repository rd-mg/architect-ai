...
func RunArgs(args []string, stdout io.Writer) error {
	...
	if len(args) > 0 {
		...
		case "version", "--version", "-v":
			_, _ = fmt.Fprintf(stdout, "architect-ai %s\n", Version)
			return nil
		case "help", "--help", "-h":
	...
	// Self-update: check for a newer architect-ai release and apply it before
...
// runUpgrade handles the `architect-ai upgrade [--dry-run] [tool...]` command.
//
// This command:
//   - Checks for available updates for managed tools (architect-ai, engram, gga)
...
//   - Skips architect-ai itself when running as a dev build (version="dev")
