...
// TestRunArgsRestoreListIsDispatched verifies that `architect-ai restore --list`
...
func TestPersistAssignmentsNoOpWhenEmpty(t *testing.T) {
	...
	statePath := filepath.Join(home, ".architect-ai", "state.json")
...
// TestVersionBeforeSystemGuards verifies that `architect-ai version` returns the
...
func TestVersionBeforeSystemGuards(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "architect-ai") {
		t.Error("version output should contain 'architect-ai'")
	}
}
...
// an error whose message suggests running 'architect-ai help'.
func TestUnknownCommandSuggestsHelp(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "architect-ai help") {
		t.Error("unknown command error should suggest 'architect-ai help'")
	}
}
