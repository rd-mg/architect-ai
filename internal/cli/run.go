...
import (
	...
	"github.com/rd-mg/architect-ai/internal/components/permissions"
	...
	"github.com/rd-mg/architect-ai/internal/components/theme"
...
var (
	...
	// AppVersion is the architect-ai version that will be written into backup manifests.
	// It is set by app.go before any CLI operation so that every backup created during
	// an install or sync records which version of architect-ai made it.
...
type prepareBackupStep struct {
	...
	// appVersion is the architect-ai version that created this backup.
...
func componentPaths(homeDir string, selection model.Selection, adapters []agents.Adapter, component model.ComponentID) []string {
	...
	for _, adapter := range adapters {
		...
		case model.ComponentPersona:
			...
			if selection.Persona == model.PersonaArchitect {
				if adapter.SupportsOutputStyles() {
					paths = append(paths, adapter.OutputStyleDir(homeDir)+"/architect.md")
					if p := adapter.SettingsPath(homeDir); p != "" {
						paths = append(paths, p)
					}
				}
