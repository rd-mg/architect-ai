...
//
// Discovery order:
//  1. Persisted state (~/.architect-ai/state.json) — written at install time.
...
func RenderSyncReport(result SyncResult) string {
	...
	if result.NoOp {
		fmt.Fprintln(&b, "architect-ai sync — no managed sync actions needed")
	...
	if result.DryRun {
		fmt.Fprintln(&b, "architect-ai sync — dry-run")
		fmt.Fprintf(&b, "Agents: %s\n", joinAgentIDs(result.Agents))
	...
	fmt.Fprintln(&b, "architect-ai sync — managed sync executed")
