...
import (
	...
	windsurfagent "github.com/rd-mg/architect-ai/internal/agents/windsurf"
...
func TestInjectCursorWritesSDDOrchestratorAndSkills(t *testing.T) {
	...
	promptPath := filepath.Join(home, ".cursor", "rules", "architect-ai.mdc")
...
func TestInjectWindsurf_WorkflowsFoundFromSubdirectory(t *testing.T) {
	...
	// Simulate running architect-ai from a subdirectory inside that project.
...
// ---------------------------------------------------------------------------
// architect agent model mirroring from sdd-orchestrator
// ---------------------------------------------------------------------------

// TestInjectOpenCodeMultiModeMirrorsOrchestratorModelToArchitect verifies that
// when sdd-orchestrator has an explicit TUI model assignment and the architect
// agent already exists in opencode.json (persona installed), the orchestrator
// model is mirrored to the architect agent.
func TestInjectOpenCodeMultiModeMirrorsOrchestratorModelToArchitect(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
	...
	// Pre-existing opencode.json with architect agent (persona installed).
	existing := `{
  "agent": {
    "architect": {
      "mode": "primary"
	...
	// architect must have the same model as sdd-orchestrator (mirrored).
	architectAgent, ok := agentMap["architect"].(map[string]any)
	if !ok {
		t.Fatal("architect agent not found or wrong type")
	}
	if m, _ := architectAgent["model"].(string); m != "openai/gpt-4o" {
		t.Fatalf("architect model = %q, want %q (should mirror sdd-orchestrator)", m, "openai/gpt-4o")
	}
}

// TestInjectOpenCodeMultiModeDoesNotInjectArchitectIfNotInstalled verifies that
// when the architect agent does NOT exist in opencode.json (persona not installed),
// the orchestrator model is NOT mirrored to a architect entry.
func TestInjectOpenCodeMultiModeDoesNotInjectArchitectIfNotInstalled(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
	...
	// architect must NOT appear — persona is not installed.
	if architectRaw, exists := agentMap["architect"]; exists {
		// If it somehow exists, it must not have a model field.
		if architectMap, ok := architectRaw.(map[string]any); ok {
			if _, hasModel := architectMap["model"]; hasModel {
				t.Fatal("architect should NOT have a model field when persona is not installed")
			}
		}
	}
}
...
func TestMergeJSONFileReturnsMergedBytes(t *testing.T) {
	...
	result, err := mergeJSONFile(path, overlay, false)
	if err != nil {
