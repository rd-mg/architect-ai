...
type InjectOptions struct {
	...
	// <!-- architect-ai:strict-tdd-mode --> marker section is injected into
	...
	// SkillIDs is an optional filter for which SDD skills to install.
	// If empty, all SDD skills are installed.
	SkillIDs []model.SkillID

	// Force enables forced overwrite of SDD assets.
	Force bool
}
...
//
// Walking upward means users can run architect-ai from any subdirectory of their
...
func Inject(homeDir string, adapter agents.Adapter, sddMode model.SDDModeID, options ...InjectOptions) (InjectionResult, error) {
	...
	if opts.StrictTDD {
		...
		writeResult, writeErr := filemerge.WriteFileAtomicWithOptions(promptPath, []byte(updated), filemerge.WriteOptions{
			Perm:  0o644,
			Force: opts.Force,
		})
		if writeErr != nil {
	...
	if adapter.Agent() == model.AgentOpenCode {
		...
		if settingsPath != "" {
			...
			agentResult, err := mergeJSONFile(settingsPath, overlayBytes, opts.Force)
			if err != nil {
				return InjectionResult{}, err
			}
			...
			for _, profile := range opts.Profiles {
				...
				profileResult, profileErr := mergeJSONFile(settingsPath, profileOverlay, opts.Force)
				if profileErr != nil {
	...
	if adapter.SupportsSkills() {
		...
		if skillDir != "" {
			...
			for _, fileName := range sharedFiles {
				...
				// 1. Try local repository source first (for development)
				if projectRoot, found := findProjectRoot(opts.WorkspaceDir); found {
					devPath := filepath.Join(projectRoot, "internal", "assets", "skills", "_shared", fileName)
					if data, err := os.ReadFile(devPath); err == nil {
						content = string(data)
					}
				}

				// 2. Try embedded assets (bundle)
				if content == "" {
				...
				if len(content) == 0 {
					return InjectionResult{}, fmt.Errorf("required SDD shared file %q: asset is empty", fileName)
				}
			...
			for _, skill := range sddSkills {
				...
				// 1. Try local repository source first (for development)
				if projectRoot, found := findProjectRoot(opts.WorkspaceDir); found {
					devPath := filepath.Join(projectRoot, "internal", "assets", "skills", skill)
					if info, err := os.Stat(devPath); err == nil && info.IsDir() {
						skillFS = os.DirFS(devPath)
						skillSourceDir = "."
					}
				}
				...
				if skillFS == nil {
					...
					if _, err := fs.Stat(assets.FS, embedDir); err == nil {
						skillFS = assets.FS
						skillSourceDir = embedDir
					}
				}

				if skillFS == nil {
					return InjectionResult{}, fmt.Errorf("required SDD skill %q: source directory not found in local source or bundle", skill)
				}

				walkErr := fs.WalkDir(skillFS, skillSourceDir, func(p string, d fs.DirEntry, err error) error {
					if err != nil {
						return err
					}
					if d.IsDir() {
					...
					if skillFS == assets.FS {
						content, readErr = assets.FS.ReadFile(p)
					} else {
						content, readErr = fs.ReadFile(skillFS, p)
					}

					if readErr != nil {
						return fmt.Errorf("required SDD skill %q file %q: asset not found: %w", skill, p, readErr)
					}
					if len(content) == 0 {
						return fmt.Errorf("required SDD skill %q file %q: asset is empty", skill, p)
					}

					relPath := p
					if skillSourceDir != "." {
						relPath = strings.TrimPrefix(p, skillSourceDir+"/")
					}
					path := filepath.Join(skillDir, skill, relPath)
					writeResult, err := filemerge.WriteFileAtomicWithOptions(path, content, filemerge.WriteOptions{
						Perm:  0o644,
						Force: opts.Force,
					})
					if err != nil {
						return fmt.Errorf("required SDD skill %q: write failed: %w", skill, err)
					}
	...
	// findProjectRoot walks upward from WorkspaceDir so architect-ai can be
	...
	if sai, ok := adapter.(agents.SubAgentCapableAdapter); ok && sai.SupportsSubAgents() {
		...
		var agentEntries []fs.DirEntry
		var agentSourcePath string
		var useFS fs.FS

		// 1. Try local repository source first (for development)
		if projectRoot, found := findProjectRoot(opts.WorkspaceDir); found {
			devPath := filepath.Join(projectRoot, "internal", "assets", embeddedDir)
			if info, err := os.Stat(devPath); err == nil && info.IsDir() {
				agentSourcePath = devPath
		...
		if len(agentEntries) == 0 {
			if entries, err := assets.FS.ReadDir(embeddedDir); err == nil {
				agentEntries = entries
				useFS = assets.FS
				agentSourcePath = embeddedDir
			}
		}

		if len(agentEntries) == 0 {
			return InjectionResult{}, fmt.Errorf("read agents source dir: not found in local source or bundle")
		}
		...
		for _, entry := range agentEntries {
			...
			var content string
			if useFS != nil {
				content = assets.MustRead(agentSourcePath + "/" + entry.Name())
			} else {
				data, err := os.ReadFile(filepath.Join(agentSourcePath, entry.Name()))
				if err != nil {
					return InjectionResult{}, fmt.Errorf("read local agent %s: %w", entry.Name(), err)
				}
				content = string(data)
			}
...
func installOpenCodePlugins(homeDir string, force bool, manifest *state.ManagedManifest) (InjectionResult, error) {
	...
	writeResult, err := filemerge.WriteFileAtomicWithOptions(pluginPath, []byte(content), filemerge.WriteOptions{
		Perm:  0o644,
		Force: force,
	})
	if err != nil {
...
func mergeJSONFile(path string, overlay []byte, force bool) (mergeJSONResult, error) {
	...
	writeResult, err := filemerge.WriteFileAtomicWithOptions(path, merged, filemerge.WriteOptions{
		Perm:  0o644,
		Force: force,
	})
	if err != nil {
...
// generalOrchestratorAsset returns the embedded asset path for the general orchestrator
// content based on the agent. Agent-specific assets take priority; generic is fallback.
...
func injectFileAppend(homeDir string, adapter agents.Adapter, force bool, manifest *state.ManagedManifest, options ...InjectOptions) (InjectionResult, error) {
	...
	if len(options) > 0 {
		...
		if resolved, err := resolvePromptTemplate(l0Content, ctx); err == nil {
			l0Content = resolved
		}
	}
	...
	if writeResult.Changed {
		state.RecordManagedSection(manifest, model.ComponentSDD, promptPath, "L0")
		state.RecordManagedSection(manifest, model.ComponentSDD, promptPath, "L1a")
		state.RecordManagedSection(manifest, model.ComponentSDD, promptPath, "L1b")
	}
...
func hasLegacyBareOrchestrator(content string) bool {
	markedIdx := strings.Index(content, "<!-- architect-ai:sdd-orchestrator -->")
	if markedIdx >= 0 {
...
//   - end at the next managed marker ("<!-- architect-ai:") if present, else EOF
...
func stripBareOrchestratorForFilePrompt(content string) string {
	if markedIdx := strings.Index(content, "<!-- architect-ai:sdd-orchestrator -->"); markedIdx >= 0 {
		prefix := content[:markedIdx]
	...
	if rel := strings.Index(content[start:], "<!-- architect-ai:"); rel >= 0 {
		end = start + rel
	}
...
const instructionsFrontmatter = "---\n" +
	...
	"description: Architect persona with SDD orchestration and Engram protocol\n" +
	"applyTo: \"**\"\n" +
	"---\n"
...
func injectMarkdownSections(homeDir string, adapter agents.Adapter, assignments map[string]model.ClaudeModelAlias, force bool, manifest *state.ManagedManifest, options ...InjectOptions) (InjectionResult, error) {
	...
	// Determine which sdd-orchestrator asset to use
	sddOrchestratorAsset := "generic/sdd-orchestrator.md"
	if adapter.Agent() == model.AgentClaudeCode {
		sddOrchestratorAsset = "claude/sdd-orchestrator.md"
	}
	content := assets.MustRead(sddOrchestratorAsset)
	...
	if hasSDDOrchestrator(existing) && !strings.Contains(existing, "<!-- architect-ai:L1a -->") {
...
func injectModelAssignments(overlayBytes []byte, assignments map[string]model.ModelAssignment, rootModelID string, existingAgentKeys map[string]bool) ([]byte, error) {
	...
	// Mirror sdd-orchestrator model to architect — both are primary conductors in OpenCode.
	// architect is defined by the persona overlay (not the SDD overlay), so we inject
	// its model field here to prevent silent runtime inheritance.
	// Guard: only inject if architect already exists in opencode.json (persona was installed)
	...
	if orchAssignment, hasOrch := assignments["sdd-orchestrator"]; hasOrch &&
		...
		existingAgentKeys["architect"] {
		if _, exists := agents["architect"]; !exists {
			agents["architect"] = map[string]any{}
		}
		if architectMap, ok := agents["architect"].(map[string]any); ok {
			architectMap["model"] = orchAssignment.FullID()
		}
	}
