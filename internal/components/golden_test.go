...
import (
	...
	"github.com/rd-mg/architect-ai/internal/components/persona"
...
func TestGoldenSDD_Cursor(t *testing.T) {
	...
	// Cursor writes SDD orchestrator to ~/.cursor/rules/architect-ai.mdc.
	rulesFile := readTestFile(t, filepath.Join(home, ".cursor", "rules", "architect-ai.mdc"))
...
func TestGoldenSDD_Kiro(t *testing.T) {
	...
	// Kiro writes SDD orchestrator to ~/.kiro/steering/architect-ai.md
...
func TestGoldenPersona_Claude_Architect(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, claudeAdapter(), model.PersonaArchitect)
	if err != nil {
		t.Fatalf("persona.Inject(claude, architect) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(claude, architect) changed = false")
	}

	claudeMD := readTestFile(t, filepath.Join(home, ".claude", "CLAUDE.md"))
	assertGolden(t, "persona-claude-architect.golden", claudeMD)

	outputStyle := readTestFile(t, filepath.Join(home, ".claude", "output-styles", "architect.md"))
	assertGolden(t, "persona-claude-architect-outputstyle.golden", outputStyle)

	settingsJSON := readTestFile(t, filepath.Join(home, ".claude", "settings.json"))
	assertGolden(t, "persona-claude-architect-settings.golden", settingsJSON)
}
...
func TestGoldenPersona_OpenCode_Architect(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, opencodeAdapter(), model.PersonaArchitect)
	if err != nil {
		t.Fatalf("persona.Inject(opencode, architect) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(opencode, architect) changed = false")
	}

	agentsMD := readTestFile(t, filepath.Join(home, ".config", "opencode", "AGENTS.md"))
	assertGolden(t, "persona-opencode-architect.golden", agentsMD)
}
...
func TestGoldenPersona_Windsurf_Architect(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, windsurfAdapter(), model.PersonaArchitect)
	if err != nil {
		t.Fatalf("persona.Inject(windsurf, architect) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(windsurf, architect) changed = false")
	}

	globalRules := readTestFile(t, filepath.Join(home, ".codeium", "windsurf", "memories", "global_rules.md"))
	assertGolden(t, "persona-windsurf-architect.golden", globalRules)
}

func TestGoldenPersona_Kiro_Architect(t *testing.T) {
	home := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("APPDATA", filepath.Join(home, "AppData", "Roaming"))

	adapter := kiroAdapter()
	result, err := persona.Inject(home, adapter, model.PersonaArchitect)
	if err != nil {
		t.Fatalf("persona.Inject(kiro, architect) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(kiro, architect) changed = false")
	}

	promptPath := adapter.SystemPromptFile(home)
	instructionsFile := readTestFile(t, promptPath)
	assertGolden(t, "persona-kiro-architect.golden", instructionsFile)
}
...
func TestGoldenCombined_Claude(t *testing.T) {
	...
	if _, err := persona.Inject(home, claudeAdapter(), model.PersonaArchitect); err != nil {
...
func TestGoldenCombined_Windsurf(t *testing.T) {
	...
	if _, err := persona.Inject(home, windsurfAdapter(), model.PersonaArchitect); err != nil {
...
func TestGoldenPersona_Antigravity_Architect(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, antigravityAdapter(), model.PersonaArchitect)
	if err != nil {
		t.Fatalf("persona.Inject(antigravity, architect) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(antigravity, architect) changed = false")
	}

	rulesFile := readTestFile(t, filepath.Join(home, ".gemini", "GEMINI.md"))
	assertGolden(t, "persona-antigravity-architect.golden", rulesFile)
}
