...
func TestRemoveMarkdownSections_RemovesOnlyManagedBlock(t *testing.T) {
	...
	if strings.Contains(updated, "architect-ai:engram-protocol") {
...
func TestRemoveManagedPersonaPreamble_PreservesManagedSuffix(t *testing.T) {
	...
	if strings.Contains(updated, "name: Architect AI Persona") || strings.Contains(updated, "## Personality") {
		t.Fatalf("managed persona preamble still present:\n%s", updated)
	}
	if !strings.HasPrefix(updated, "<!-- architect-ai:sdd-orchestrator -->") {
...
func TestMarkdownCleanup_OnRealFileWithTempDir(t *testing.T) {
	...
	if strings.Contains(final, "architect-ai:engram-protocol") {
