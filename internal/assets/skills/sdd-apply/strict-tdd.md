# Strict TDD Module — Apply Phase

> **Loaded ONLY when Strict TDD Mode enabled AND test runner available.**
> If reading this, orchestrator verified both conditions. Follow every instruction.

## TDD Philosophy

TDD is **software design driven by tests**. Write test describing what code SHOULD do, then write minimum code to make it real. Tests design API, contracts, behavior. Code is side effect of tests.

### Three Laws

1. Do NOT write production code until failing test exists
2. Do NOT write more test than necessary to fail
3. Do NOT write more code than necessary to pass the test

## TDD Implementation Cycle

For EVERY assigned task, follow this cycle strictly:

```
FOR EACH TASK:
├── 0. SAFETY NET (only if modifying existing files)
│   ├── Run existing tests for files being modified
│   ├── Capture baseline: "{N} tests passing"
│   ├── If any FAIL → STOP, report as "pre-existing failure"
│   │   (do NOT fix pre-existing failures — report to orchestrator)
│   └── Baseline proves you did not break existing code
│
├── 1. UNDERSTAND
│   ├── Read task description
│   ├── Read relevant spec scenarios (ARE your acceptance criteria)
│   ├── Read design decisions (CONSTRAIN your approach)
│   ├── Read existing code and test patterns (match style)
│   └── Determine test layer (see "Choosing Test Layer")
│
├── 2. RED — Write failing test FIRST
│   ├── Write test(s) describing expected behavior from spec
│   ├── Prefer pure functions (no side effects = easy to test)
│   ├── Test MUST reference production code that does NOT exist yet
│   │   (guarantees failure — no need to execute to confirm)
│   ├── If production code/function already exists:
│   │   └── Write test for NEW behavior NOT yet implemented
│   └── GATE: Do NOT proceed to GREEN until test is written
│
├── 3. GREEN — Write MINIMUM code to pass
│   ├── Implement ONLY what failing test needs
│   ├── Fake It is VALID (hardcoded return values OK)
│   ├── EXECUTE tests → must PASS
│   │   ├── Passed → proceed to TRIANGULATE or REFACTOR
│   │   └── Failed → fix implementation, NOT the test
│   └── GATE: Do NOT proceed until GREEN confirmed by execution
│
├── 4. TRIANGULATE (MANDATORY for most tasks)
│   ├── DEFAULT: triangulation REQUIRED. Need compelling reason to skip.
│   ├── Add second test case with DIFFERENT inputs/expected outputs
│   ├── EXECUTE tests → if Fake It breaks (hardcoded no longer works):
│   │   └── Generalize to real logic (this is the whole point)
│   ├── Repeat until ALL spec scenarios for this task are covered
│   ├── Each pass: write test → run → fix implementation
│   ├── MINIMUM: at least 2 test cases per behavior (happy path + edge case)
│   │   ├── Test with data producing NON-EMPTY/NON-TRIVIAL result
│   │   └── Test exercising DIFFERENT code path
│   ├── WATCH OUT for GREEN passing trivially:
│   │   ├── Test passes because component isn't rendered → NOT real GREEN
│   │   ├── Test passes because loop iterates 0 times → NOT real GREEN
│   │   ├── Test passes because setup doesn't trigger code path → NOT real GREEN
│   │   └── Real GREEN: production code RAN and produced expected output
│   ├── Skip triangulation ONLY when ALL true:
│   │   ├── Task is purely structural (config file, constant, type export)
│   │   ├── Literally ONE possible output (no branching, no logic)
│   │   └── Explicitly note "Triangulation skipped: {reason}" in evidence table
│   └── GATE: All spec scenarios for task must have tests before REFACTOR
│
├── 5. REFACTOR — Improve without changing behavior
│   ├── Extract constants (eliminate magic numbers)
│   ├── Extract functions (reduce cyclomatic complexity)
│   ├── Improve naming, remove duplication
│   ├── Push toward pure functions where feasible
│   ├── Apply Boy Scout Rule: leave code cleaner than found
│   ├── EXECUTE tests after EACH refactoring step → must STILL PASS
│   │   ├── Still passing → safe, continue
│   │   └── Failed → REVERT that step, try smaller
│   └── GATE: Tests green after EVERY refactoring change
│
├── 6. Mark task complete [x]
└── 7. Note deviations or issues discovered
```

## Choosing Test Layer

Based on testing capabilities cached in Engram (`sdd/{project}/testing-capabilities`):

```
Determine test layer by WHAT task does:
├── Pure logic, utility, calculation, data transformation
│   └── Unit test (always available if test runner exists)
│
├── Component rendering, user interaction, state changes
│   ├── IF integration tools available → Integration test
│   └── IF NOT → Unit test with mocks (degrade gracefully)
│
├── Multi-component flow, API interaction, context/provider behavior
│   ├── IF integration tools available → Integration test
│   └── IF NOT → Unit test with mocks
│
├── Critical business flow, full user journey, cross-page navigation
│   ├── IF E2E tools available → E2E test
│   ├── IF NOT but integration available → Integration test
│   └── IF neither → Unit test (degrade gracefully)
│
└── Default: Unit test (always the fallback)
```

**Key rule**: Use HIGHEST available layer fitting task. NEVER skip task because layer unavailable — degrade to next available layer.

## Test Execution

```
Read test command from:
├── Cached capabilities → test_runner.command (fastest — already detected)
├── openspec/config.yaml → rules.apply.test_command (override)
└── Fallback: detect from package.json/pyproject.toml/go.mod

When executing tests during TDD:
├── Run ONLY relevant test file, not entire suite
│   ├── JS/TS: {runner} {test-file-path} (e.g., pnpm vitest run src/utils/tax.test.ts)
│   ├── Python: pytest {test-file-path}
│   ├── Go: go test ./{package}/... -run {TestName}
│   └── Adapt to runner's CLI
├── Keeps cycle FAST
└── Full suite runs happen in sdd-verify, not here
```

## Pure Function Preference

When writing production code in GREEN/TRIANGULATE, prefer pure functions:

```
 PREFER (pure — easy to test):
function calculateDiscount(price: number, quantity: number): number {
  return quantity >= 5 ? price * quantity * 0.1 : 0
}

 AVOID (impure — hard to test):
function calculateDiscount(item: Item) {
  globalState.lastDiscount = item.price * 0.1  // side effect
  updateDOM()                                   // side effect
  return globalState.lastDiscount
}
```

**Why**: Pure functions deterministic (same input → same output), no side effects, trivially testable. TDD naturally pushes toward pure functions — embrace it.

## Approval Testing (for refactoring existing code)

When task involves REFACTORING existing code:

```
BEFORE touching production code:
├── 1. Identify existing behavior to preserve
├── 2. Write "approval tests" capturing current behavior:
│   ├── Call function with known inputs
│   ├── Assert CURRENT outputs (even if ugly/wrong)
│   └── These tests document what code does NOW
├── 3. Run approval tests → must PASS (describe current reality)
├── 4. NOW refactor production code
├── 5. Run approval tests again → must STILL PASS
│   ├── Passing → refactoring preserved behavior
│   └── Failing → refactoring broke something, revert
└── 6. If spec says behavior should CHANGE:
    ├── Update approval test to reflect NEW expected behavior
    ├── Run → test FAILS (RED — new behavior not implemented)
    └── Implement new behavior → GREEN
```

## Return Summary Extension

When Strict TDD Mode active, return summary MUST include:

```markdown
### TDD Cycle Evidence
| Task | Test File | Layer | Safety Net | RED | GREEN | TRIANGULATE | REFACTOR |
|------|-----------|-------|------------|-----|-------|-------------|----------|
| 1.1 | `path/test.ext` | Unit |  5/5 |  Written |  Passed |  3 cases |  Clean |
| 1.2 | `path/test.ext` | Integration | N/A (new) |  Written |  Passed |  Single |  Clean |
| 1.3 | `path/test.ext` | Unit |  2/2 |  Written |  Passed |  2 cases |  None needed |

### Test Summary
- **Total tests written**: {N}
- **Total tests passing**: {N}
- **Layers used**: Unit ({N}), Integration ({N}), E2E ({N})
- **Approval tests** (refactoring): {N} or "None — no refactoring tasks"
- **Pure functions created**: {N}
```

**Column definitions**:
- **Safety Net**: Pre-existing tests run before modifying files. "N/A (new)" for new files.
- **RED**: Test written first, referencing code that doesn't exist yet. Always " Written".
- **GREEN**: Tests executed and passing after minimal implementation. Must show execution result.
- **TRIANGULATE**: Additional test cases added to force real logic. " Single" if spec has only one scenario.
- **REFACTOR**: Code improved with tests still passing. " None needed" if code already clean.

## Assertion Quality Rules (MANDATORY)

**Every assertion must verify REAL behavior.** Test passing without exercising production logic is worse than no test — gives false confidence.

### Banned Assertion Patterns (NEVER write these)

```
# TRIVIAL ASSERTIONS — test proves nothing
expect(true).toBe(true)              #  Tautology
expect(false).toBe(false)            #  Tautology
expect(1).toBe(1)                    #  Tautology — no production code involved
assert True                          #  Always passes
assert 1 == 1                        #  Always passes

# EMPTY COLLECTION ASSERTIONS without setup context
expect(result).toEqual([])           #  ONLY valid if setup conditions for empty
expect(result).toHaveLength(0)       #  Same — why empty? Did production code run?
assert len(result) == 0              #  Same — prove emptiness from real logic
assert result == []                  #  Same

# TYPE-ONLY ASSERTIONS — proves existence, not behavior
expect(result).toBeDefined()         #  Alone is useless — WHAT is the value?
expect(result).not.toBeNull()        #  Alone is useless — assert actual value
expect(typeof result).toBe('object') #  Alone is useless — what does object contain?
assert result is not None            #  Alone — assert what result actually IS

# GHOST LOOP — assertion inside loop iterating 0 times
const items = screen.queryAllByTestId("item");  // returns []
for (const item of items) {
  expect(item).toHaveTextContent("value");       #  NEVER EXECUTES — dead code
}
# FIX: assert collection non-empty FIRST, or set up data so it IS non-empty:
expect(items).toHaveLength(3);                   #  Proves items exist
for (const item of items) { ... }                #  Now loop actually runs

# INCOMPLETE TDD CYCLE — GREEN without TRIANGULATE
# If GREEN test passes because setup doesn't exercise code path,
# NOT done. MUST triangulate with setup that DOES exercise it.
# Example: testing "search doesn't update until Enter" but component
# receiving search never rendered → test proves nothing.
# FIX: add test where component IS rendered and verify behavior.
```

### What Makes a REAL Assertion

Every test assertion must satisfy ALL:
1. **Calls production code** — invokes function/method/component from implementation
2. **Asserts specific output** — compares against concrete expected value from spec
3. **Would FAIL if production code were wrong** — changing implementation logic breaks THIS test

```
#  REAL assertions — production code determines result
expect(calculateDiscount(100, 10)).toBe(10)       # Real input → real output
expect(screen.getByText('Welcome, John')).toBeInTheDocument()  # Rendered from data
assert result[0].status == "FAIL"                  # Specific finding from check execution
assert response.status_code == 403                 # Real HTTP response from endpoint
expect(result).toHaveLength(3)                     # AND you set up exactly 3 items
```

### Empty Collection Rule

`expect(result).toEqual([])` or `assert len(result) == 0` ONLY valid when:
1. Setup specific precondition that SHOULD produce empty result (e.g., no matching records)
2. Production code actually ran and filtered/processed to arrive at empty
3. Companion test with different setup produces NON-EMPTY result (triangulation)

Cannot explain WHY result is empty based on setup → assertion is trivial.

### Smoke Test Rule

Test only rendering component without asserting output is NOT valid:

```
#  SMOKE TEST ONLY — proves nothing about behavior
render(<MyComponent data={mockData} />);
expect(screen.getByTestId("wrapper")).toBeInTheDocument();  # Just proves it rendered

#  BEHAVIORAL TEST — proves what component DOES with data
render(<MyComponent data={mockData} />);
expect(screen.getByText("Expected Title")).toBeInTheDocument();  # Verifies output from data
expect(screen.getByRole("button")).toHaveTextContent("Submit");  # Verifies real content
```

"Renders without crash" is smoke test. NOT unit test, NOT integration test, does NOT count toward TDD coverage. Smoke test must be accompanied by real behavioral assertions.

### Mock Hygiene Rules

**If you need more mocks than assertions, testing at WRONG level.**

```
Mock/assertion ratio guide:
├── ≤ 3 mocks for test file →  Healthy — focused test
├── 4–6 mocks →  Consider extracting logic to pure function
├── 7+ mocks →  STOP — testing at wrong layer
│   ├── Extract logic under test to PURE FUNCTION, test without mocks
│   ├── OR move test to integration/E2E layer where real dependencies exist
│   └── NEVER write 10+ mocks to verify one-line transformation
```

**Extract-Before-Mock Rule**: If behavior to test is data transformation, mapping, filtering, or conditional logic (e.g., `MUTED → FAIL` status), EXTRACT to pure function FIRST, then test pure function directly. No mocks needed.

```
#  BAD: 15 mocks to test one-line status conversion
vi.mock("next/navigation", ...);
vi.mock("next/link", ...);
vi.mock("@/components/shadcn", ...);
// ... 12 more mocks ...
render(<StatusCell row={mutedRow} />);
expect(screen.getByText("FAIL")).toBeInTheDocument();

#  GOOD: extract and test logic directly
// In production code:
export function resolveDisplayStatus(status: string, isMuted: boolean): string {
  return status === "MUTED" ? "FAIL" : status;
}

// In test — ZERO mocks needed:
expect(resolveDisplayStatus("MUTED", true)).toBe("FAIL");
expect(resolveDisplayStatus("PASS", false)).toBe("PASS");
```

### Implementation Detail Coupling Rule

Tests must assert **behavior visible to user**, not internal implementation details:

```
#  COUPLED TO IMPLEMENTATION — breaks on any style refactor
expect(element.className).toContain("text-xs");
expect(element.className).toContain("-mt-2.5");
expect(element.className).toContain("border-border-error-primary");
expect(element.style.color).toBe("red");

#  COUPLED TO INTERNALS — breaks when implementation changes
expect(mockService.mock.calls.length).toBe(3);  # Why 3? Brittle.
expect(component.state.isLoading).toBe(true);    # Internal state, not behavior.

#  BEHAVIORAL — survives refactors, tests what users see
expect(screen.getByText("Error: Payment failed")).toBeInTheDocument();
expect(screen.getByRole("alert")).toHaveTextContent("Risk:");
expect(screen.getByRole("button")).toBeDisabled();
```

**CSS class assertions NEVER valid test assertions.** To verify visual styling:
1. Test **semantic outcome** (e.g., element has `role="alert"`, text visible, button disabled)
2. OR use visual regression tool / E2E screenshot comparison
3. NEVER assert specific Tailwind/CSS class names — implementation details

## Rules (Strict TDD)

- NEVER write production code before writing test — ONE rule that cannot be broken
- NEVER skip GREEN execution gate — MUST run tests and confirm pass
- NEVER skip triangulation when spec defines multiple scenarios — hardcoded Fake It must be forced out
- NEVER write trivial assertions (see Banned Assertion Patterns) — WORSE than no test
- ALWAYS verify every assertion CALLS production code and asserts SPECIFIC expected value
- ALWAYS run Safety Net before modifying existing files — protect what works
- ALWAYS report TDD Cycle Evidence table — verify phase checks it
- If test runner execution fails for infrastructure reasons (not test failures), report as "Blocked" and continue to next task
- Prefer pure functions — but don't force where doesn't fit (e.g., React components with state)
- For refactoring tasks, ALWAYS write approval tests before touching code
- Run ONLY relevant test file during cycle, not full suite
