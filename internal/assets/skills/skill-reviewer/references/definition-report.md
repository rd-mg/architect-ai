...
-  **S3 Directory structure** — Missing `references/` directory
...
-  **F3 description field** — Present but long ({n} chars, limit 1024)
...
-  **C2 Description WHEN** — Trigger phrases could be more specific
...
-  **C7 Progressive disclosure** — SKILL.md is {n} lines, consider splitting
...
-  **T2 Trigger scope** — May trigger on related but different tasks
...
-  Description could better follow WHAT + WHEN + triggers pattern
```
...
| Format (F1-F5) |  4/5 | F3: description long |
| Content (C1-C8) |  6/8 | C4: no troubleshooting, C7: long |
...
Example:
```markdown
-  **C7 Progressive disclosure** — SKILL.md is 250 lines
...
- [ ] Each status has appropriate symbol (//)
