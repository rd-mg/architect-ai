...
**Before** responding to ANY user message, scan free-text for SDD intent. Orchestrator must detect intent even without slash commands.
...
2. If user confirms and command needs change-name and none is in message → ASK for one:
...
On user's FIRST SDD-triggering message of session (via slash command or intent resolution), orchestrator MUST collect three inputs BEFORE delegating:
...
Injection order: mandatory skills FIRST, then task-matched skills. Mandatory skills carry `bridge: always` in their frontmatter; skill resolver respects this marker.
...
**BEFORE** delegating design or implementation task, perform simplicity check:
- **Abstraction Gate**: If task proposes new interface, wrapper, or base class, ensure at least 2 distinct implementations planned. If only 1, enforce direct implementation.
- **Scale Check**: If solution handles theoretical 'future' loads or features not in spec, REJECT and simplify.
...
**BEFORE** delegating to sub-agent, MUST perform:
...
**AFTER** receiving sub-agent response, MUST perform:
...
If `artifact_store == none` and prior `sdd-apply` launched this session, emit warning exactly once per session: "Your apply progress is NOT persisted in `none` mode. If you need to pause, re-run with `engram` or `openspec` next session."
...
Orchestrator also persists session stats to Engram under `metering/{project}/{session-id}` so `sdd-archive` can include them in final report.
