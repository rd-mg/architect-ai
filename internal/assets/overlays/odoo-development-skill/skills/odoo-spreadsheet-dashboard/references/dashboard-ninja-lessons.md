...
Dashboard Ninja is an Odoo App Store product for advanced dashboards. This reference extracts
...
1. **KPI row first** — place all scorecards in a horizontal row at the top before charts.
2. **Current period vs. previous** — every KPI should have a `baseline` comparison (MoM or YoY).
...
6. **Filter-first UX** — place company + date filters at the global filter bar; avoid per-pivot domain clutter.
...
- Do not use raw SQL by default — it bypasses Odoo access control.
- Do not duplicate global filter conditions in every pivot domain — it defeats the filter mechanism.
- Do not put calculated grids on the Dashboard sheet — keep them on the hidden Data sheet.
