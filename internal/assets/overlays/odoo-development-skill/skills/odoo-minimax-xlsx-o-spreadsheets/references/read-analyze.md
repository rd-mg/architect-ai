...
> Reference for READ path. Use `xlsx_reader.py` for structure discovery and data quality auditing,
> then pandas for custom analysis. **Never modify source file.**

---

## When to Use This Path

User asks to read, analyze, view, summarize, extract, or answer questions about Excel/CSV file's contents,
without requiring file modification. If modification needed, hand off to `edit.md`.
...
Run `xlsx_reader.py` first. Handles format detection, encoding fallback, structure exploration, and data quality audit:
...
Supported formats: `.xlsx`, `.xlsm`, `.csv`, `.tsv`. Script tries multiple encodings for CSV (utf-8-sig, gbk, utf-8, latin-1).

### Step 2 — Custom Analysis with pandas

Load data and perform analysis user requests:
...
**Header handling** (when default `header=0` doesn't work):
...
If user specifies output file path, write results to it (highest priority). Format report as:

```
## Analysis Report: {filename}
### File Overview     — format, sheets, row counts
### Data Quality      — nulls, duplicates, mixed types (or "no issues")
### Key Findings      — direct answer to user's question
...
**Critical**: Never open with `data_only=True` then `save()` — permanently destroys all formulas.

---

## Prohibitions

- Never modify source file (no `save()`, no XML edits)
- Never report formula NaN as "data is zero" — explain it's formula cache issue
- Never report pandas indices as Excel row numbers
- Never make speculative conclusions unsupported by data
