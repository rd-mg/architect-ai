...
# Odoo 19 Architecture Constants
ODOO_SUPPORTED_FONTS = {"Arial"}
ODOO_SUPPORTED_ALIGN_H = {"left", "center", "right"}
ODOO_SUPPORTED_ALIGN_V = {"top", "center", "bottom"}
...
def audit_odoo_compatibility(xlsx_path: str) -> dict:
    """
    Perform deep audit of XLSX features against Odoo 19 import limitations.
    """
    audit_results = {
        "is_compatible": True,
        "violations": [],
    ...
    try:
        ...
        return {"is_compatible": False, "violations": [{"type": "file_error", "message": str(e)}]}

    with z:
        # 1. Audit Styles (Fonts, Fills, Borders)
        if "xl/styles.xml" in z.namelist():
            styles_xml = z.read("xl/styles.xml")
            styles = ET.fromstring(styles_xml)
            
            # Fonts
            for i, font in enumerate(styles.findall(f".//{NSP}font")):
                name_elem = font.find(f"{NSP}name")
                if name_elem is not None:
                    font_name = name_elem.get("val")
                    if font_name not in ODOO_SUPPORTED_FONTS:
                        audit_results["violations"].append({
                            "type": "unsupported_font",
                            "font": font_name,
                            "index": i,
                            "fix": "Odoo 19 only supports Arial."
                        })
            
            # Fills (No gradients)
            for i, fill in enumerate(styles.findall(f".//{NSP}fill")):
                gradient = fill.find(f"{NSP}gradientFill")
                if gradient is not None:
                    audit_results["violations"].append({
                        "type": "unsupported_fill",
                        "fill_index": i,
                        "detail": "Gradient fills are not supported by Odoo 19 parser."
                    })
            
            # Borders (No diagonal)
            for i, border in enumerate(styles.findall(f".//{NSP}border")):
                diagonal = border.get("diagonalUp") or border.get("diagonalDown")
                if diagonal:
                    audit_results["violations"].append({
                        "type": "unsupported_border",
                        "border_index": i,
                        "detail": "Diagonal borders are blocked in Odoo exports."
                    })

        # 2. Audit Worksheets (Alignments, Newlines, Conditional Formats)
        sheet_files = get_sheet_files(z)
        for rid, ws_file in sheet_files.items():
            if ws_file not in z.namelist(): continue
            ws_xml = z.read(ws_file)
            ws = ET.fromstring(ws_xml)
            
            # Conditional Formatting
            for cf in ws.findall(f".//{NSP}conditionalFormatting"):
                for rule in cf.findall(f"{NSP}cfRule"):
                    rule_type = rule.get("type")
                    if rule_type in {"aboveAverage", "top10", "dataBar", "duplicateValues"}:
                        audit_results["violations"].append({
                            "type": "unsupported_cf",
                            "cf_type": rule_type,
                            "sheet": ws_file,
                            "fix": "Odoo 19 ignores this CF rule type."
                        })
                    
                    if rule_type == "iconSet":
                        icons = rule.findall(f".//{NSP}cfvo")
                        if len(icons) > 3:
                            audit_results["violations"].append({
                                "type": "truncated_iconset",
                                "count": len(icons),
                                "fix": "Odoo will truncate to 3 icons. Panel may crash if empty."
                            })

    if audit_results["violations"]:
        audit_results["is_compatible"] = False
...
def main() -> None:
    ...
    # Odoo Audit sub-report
    print("\n── Odoo 19 Compatibility Audit ──")
    audit = audit_odoo_compatibility(args_clean[0])
    if audit["is_compatible"]:
        print(" [OK] XLSX is fully compatible with Odoo 19 o-spreadsheet import.")
    else:
        for v in audit["violations"]:
            print(f" [BLOCK] {v['type']}: {v.get('detail', v.get('fix', ''))}")
