# Agent Teams Lite — Spec-Driven Development (SDD) Orchestrator Core (OpenCode)
...
OpenCode delegation syntax: sub-agents are defined in `opencode.json` with `mode: subagent`. Profile-suffixed variants (e.g. `sdd-explore-cheap`) are routed through the profile-specific orchestrator.
...
3. Copy compact rule blocks into `## Project Standards (auto-resolved)` BEFORE task-specific instructions
...
Shared under `~/.config/opencode/skills/_shared/`:
