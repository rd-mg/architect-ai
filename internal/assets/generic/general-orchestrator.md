...
### Fallback Execution (No Delegation)
If you are running in an environment without native sub-agent delegation (e.g., Gemini CLI, VS Code Copilot):
You MUST execute the task **inline**. You will assume the persona of the target agent, inject the Required Postures into your own context, and execute the task using the rules defined in `skills/_shared/general-phase-common.md`.
