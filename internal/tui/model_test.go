...
func TestStartUninstall_FullRemoveHomebrewManagedBinaryAddsManualAction(t *testing.T) {
	...
	restoreExec := setOSExecutableForTest("/opt/homebrew/bin/architect-ai", nil)
	defer restoreExec()
	...
	if !strings.Contains(msg.Result.ManualActions[0], "brew uninstall architect-ai") {
...
func TestStartUninstall_FullRemoveNonBrewRemovesBinary(t *testing.T) {
	...
	restoreExec := setOSExecutableForTest("/tmp/architect-ai", nil)
	defer restoreExec()
	...
	if removedPath != "/tmp/architect-ai" {
		t.Fatalf("os.Remove path = %q, want %q", removedPath, "/tmp/architect-ai")
	}
}
...
func makeDetectionWithAgents(present ...string) system.DetectionResult {
	known := []string{"claude-code", "opencode", "cursor", "vscode-copilot", "codex", "antigravity", "windsurf", "qwen-code"}
