...
func TestExecute_NoopWhenNothingIsExecutable(t *testing.T) {
	results := []update.UpdateResult{
		makeResult("architect-ai", update.UpToDate, "1.0.0", "1.0.0", update.InstallBinary),
...
func TestExecute_DevBuildOnlyNoBackupCreated(t *testing.T) {
	...
	results := []update.UpdateResult{
		makeResult("architect-ai", update.DevBuild, "dev", "1.0.0", update.InstallBinary),
	}
...
// architect-ai with DevBuild status must appear in Results as UpgradeSkipped
...
func TestExecute_DevBuildIsSkipped(t *testing.T) {
	...
	// architect-ai (DevBuild) MUST appear as UpgradeSkipped with a ManualHint.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "architect-ai" {
			r := report.Results[i]
			devResult = &r
		}
	}
	if devResult == nil {
		t.Fatalf("architect-ai (DevBuild) must appear in Results — was not found")
	}
	if devResult.Status != UpgradeSkipped {
		t.Errorf("architect-ai DevBuild Status = %q, want UpgradeSkipped", devResult.Status)
	}
	if devResult.ManualHint == "" {
		t.Errorf("architect-ai DevBuild ManualHint must be non-empty")
	}
...
// A DevBuild tool (e.g. architect-ai with version="dev") MUST appear in UpgradeReport.Results
...
func TestExecute_DevBuildSurfacedAsSkipped(t *testing.T) {
	...
	// architect-ai (DevBuild) MUST appear in results as UpgradeSkipped.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "architect-ai" {
			r := report.Results[i]
	...
	if devResult == nil {
		t.Fatalf("architect-ai DevBuild must appear in Results as UpgradeSkipped, but was not found")
	}
	...
	if devResult.ManualHint == "" {
		t.Errorf("architect-ai DevBuild ManualHint must be non-empty — should explain dev/source build")
	}
...
func TestExecute_ManualFallbackSurfacedAsSkippedNotFailed(t *testing.T) {
	...
	results := []update.UpdateResult{
		makeResult("architect-ai", update.UpdateAvailable, "1.0.0", "1.5.0", update.InstallBinary),
	}
	results[0].UpdateHint = "See https://github.com/rd-mg/releases"
