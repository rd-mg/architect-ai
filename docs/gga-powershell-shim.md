...
When `architect-ai` installs GGA on Windows, it now installs a `gga.ps1` wrapper
...
check — re-running `architect-ai install` is idempotent.
...
`architect-ai install`**. In the final design this was moved to **runtime** — the `.ps1`
