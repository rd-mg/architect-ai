...
This document explains how architect-ai integrates with **Kiro IDE** and what is installed in your local Kiro configuration.

## Overview

architect-ai supports Kiro as a **native-subagent** platform (`kiro-ide`).

When configured, architect-ai installs:

| Artifact | Path |
|----------|------|
| Steering file | `~/.kiro/steering/architect-ai.md` |
...
> **Auto-install not supported.** Kiro must be installed manually before running architect-ai.
...
architect-ai uses **two signals** to detect Kiro:
...
In practice: **the installer detects Kiro from `~/.kiro`**, not from `PATH`. If you have Kiro installed but `~/.kiro` hasn't been created yet (e.g., before first launch), run Kiro once to initialize its config dir, then re-run `architect-ai install`.
...
This follows the same SDD architecture used in architect-ai: orchestrator coordinates, phase agents execute, Engram persists artifacts across phases.
...
Kiro has a built-in spec workflow that architect-ai leverages. For medium and large changes, the orchestrator will use native Kiro artifacts at:
...
The steering file written by architect-ai uses the following frontmatter:
...
| Steering file | `%USERPROFILE%\.kiro\steering\architect-ai.md` |
...
- **Steering, skills, and native agents** → `~/.kiro/` (or `%USERPROFILE%\.kiro\` on Windows)
  - `~/.kiro/steering/architect-ai.md` — orchestrator persona
...
If architect-ai skills or steering are missing, check `~/.kiro/skills/` and `~/.kiro/steering/`.
