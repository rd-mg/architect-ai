...
- **GGA on Windows** works from both Git Bash and PowerShell. architect-ai installs a `gga.ps1` shim that automatically delegates to Git Bash, so no manual shell switching is required.
...
Get-FileHash .\architect-ai_<VERSION>_windows_amd64.zip -Algorithm SHA256
