...
```
cmd/architect-ai/             CLI entrypoint
internal/
...
architect-ai install --dry-run --agent claude-code --preset minimal

# Dry-run smoke test (Windows PowerShell)
architect-ai.exe install --dry-run --agent claude-code --preset minimal
```
...
| | Gentleman.Dots | Architect AI Stack |
|--|---------------|-----------------|
...
Install Gentleman.Dots first for your dev environment, then Architect AI Stack for the AI layer on top.
